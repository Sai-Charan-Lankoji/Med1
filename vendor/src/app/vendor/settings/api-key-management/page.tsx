...
const PublishableApiKeysTable = () => {
  ...
  const companyName = sessionStorage.getItem('company_name').trim()
  ...
  console.log("PublishableApiKeys:", publishableapikeyData?.[0]);
  const Data = publishableapikeyData?.[0];
  const matchingRecord = Data?.filter(record => record.title == companyName);

console.log("Hi ",matchingRecord);
              ...
              {matchingRecord?.length === 0 ? (
                ...
                <div className="overflow-x-auto shadow-sm border border-gray-200 rounded-lg">
                  <Table className="min-w-full bg-transparent">
                    ...
                    <Table.Body>
                      {matchingRecord?.map((apiKey, index) => (
