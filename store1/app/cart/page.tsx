...
const CartPage = () => {
  ...
  const handleProceedToCheckout = () => {
    ...
    // Store selected item IDs in localStorage and navigate to /order
        ...
        <div className="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              <Link href="/auth">
                <Button className="bg-white text-blue-600 hover:bg-blue-50">Sign In</Button>
              </Link>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100">
                ...
                {allCartItems.length > 0 && (
                  <div className="flex justify-between space-x-4">
                    <Button onClick={handleSelectAll} variant="outline" disabled={loading}>
                      {isAllSelected ? "Unselect All" : "Select All"}
                    </Button>
                    <Button onClick={handleClearCart} variant="destructive" disabled={loading}>
              ...
              {allCartItems.length === 0 ? (
                <div className="p-12 text-center">
                  ...
                  <Link href="/" className="inline-flex items-center bg-black text-white px-8 py-3 rounded-lg hover:bg-gray-800 transition">
                    Continue Shopping
                  </Link>
                </div>
              ) : (
                    ...
                    return (
                      <div key={item.id} className="p-6 flex items-start space-x-4">
                        <input
                          ...
                          className="mt-2 w-4 h-4 text-blue-600 rounded border-gray-500"
                          disabled={loading}
                        />
                                ...
                                {isDesignable ? (
                                  viewMode === "apparel" ? (
                                    designItem.designs?.map((design, index) => (
                                      <div
                                        key={index}
                                        className={`relative w-16 h-20 cursor-pointer transition-all ${index === mainDesignIndex ? "ring-2 ring-gray-700" : "hover:ring-2 hover:ring-gray-300"}`}
                                    ...
                                    currentDesign?.uploadedImages?.map((image, index) => (
                                      <button
                                        key={index}
                                        className={`aspect-square relative rounded-md overflow-hidden ${index === currentUploadedImageIndex ? "ring-2 ring-black" : "ring-1 ring-gray-200 hover:ring-gray-300"}`}
                          ...
                          <div className="md:w-1/2 flex flex-col">
                            <div className="flex justify-between items-start">
                              ...
                              <Button
                                ...
                                disabled={loading}
                              >
                                <FaTrash className="w-4 h-4 text-gray-400 hover:text-red-500" />
                              </Button>
                            </div>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 lg:sticky lg:top-8">
                ...
                {selectedItems.size > 0 ? (
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between text-gray-600">
                      <span>Subtotal</span>
                      <span className="font-medium text-black">${calculateSelectedTotals().subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-gray-600">
                      <span>Tax (10%)</span>
                      <span className="font-medium text-black">${calculateSelectedTotals().taxAmount.toFixed(2)}</span>
                    </div>
                    <div className="pt-4 border-t border-gray-100">
                      <div className="flex justify-between">
                        <span className="text-black text-lg font-bold">Total</span>
                        <span className="text-black text-lg font-bold">${calculateSelectedTotals().total.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                ) : (
                  <p className="text-gray-500 text-center">Please select items to proceed</p>
                )}
                <Button
                  className="w-full mt-6 bg-gray-600 hover:bg-gray-700 rounded-lg text-white shadow-md" 
