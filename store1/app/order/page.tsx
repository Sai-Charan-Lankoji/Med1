...
import { Plus, CreditCard, Loader2, ChevronLeft, ChevronRight, Eye, ImageIcon, MapPin, Home, Building, ShoppingBag, Check, AlertCircle } from "lucide-react";
...
const OrderPage = () => {
  ...
  const { addresses, newAddress, setNewAddress, handleAddAddress } = useAddresses(customerId);
  ...
  const [hsnTaxDetails, setHsnTaxDetails] = useState<{
    hsn_code: string;
  ...
  const FIXED_GST_PERCENTAGE = 12; // Fallback for designable items

  // Fetch product details for standardCartItems from stock data
  const fetchProductDetails = async (productId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/stock/${productId}`, { method: "GET" });
      ...
      return { hsnCode: FIXED_HSN_CODE, gstPercentage: FIXED_GST_PERCENTAGE }; // Fallback values
    }
  };

  const standardCartItems = useMemo(() => getStandardCartItems(), [getStandardCartItems]);
  const enrichedStandardCartItems = useMemo(() => {
    return standardCartItems.map(async (item) => {
      const { hsnCode, gstPercentage } = await fetchProductDetails(item.product_id);
    ...
    if (authToken && isLoadingCart && !loading && customerId) {
      fetchCartData()
        .then(async () => {
          ...
          const resolvedStandardItems = await Promise.all(enrichedStandardCartItems);
          const allItems = [...enrichedDesignableCartItems, ...resolvedStandardItems];
  ...
  }, [authToken, fetchCartData, loading, isLoadingCart, enrichedDesignableCartItems, enrichedStandardCartItems, customerId]);

  useEffect(() => {
    if (addresses.length > 0 && !selectedShippingAddressId) {
      const shippingAddresses = addresses.filter((addr) => addr.address_type === "shipping");
      if (shippingAddresses.length > 0) setSelectedShippingAddressId(shippingAddresses[0].id);
    }
    if (addresses.length > 0 && !selectedBillingAddressId) {
      const billingAddresses = addresses.filter((addr) => addr.address_type === "billing");
      if (billingAddresses.length > 0) setSelectedBillingAddressId(billingAddresses[0].id);
    }
  ...
  const calculateTotals = () => {
    ...
    selectedItems.forEach((item) => {
      ...
      hsnMap[hsnCode].totalTax = hsnMap[hsnCode].cgst + hsnMap[hsnCode].sgst + hsnMap[hsnCode].igst;
    });
    ...
    const totalTaxAmount = taxDetails.reduce((sum, detail) => sum + detail.totalTax, 0);
  ...
  const handleOrderSubmit = async () => {
    ...
    const orderData: OrderData = {
      line_items: selectedItems.map((item) => {
        ...
        const price = isDesignable && designItem.designs?.length ? designItem.designs.length * 100 : standardItem.price || 100;
    ...
    if (paymentMethod === "phonepe") {
      try {
        ...
        if (response.status === 429) {
          ...
          setError(`Order ${paymentData.orderId} placed, but payment failed due to too many requests. Retry after ${retryAfter} seconds.`);
        ...
        if (!paymentData.success) {
          setError(`Order ${paymentData.orderId} placed, but payment failed: ${paymentData.message || "Unknown error"}`);
  ...
  const renderAddressForm = (type: "shipping" | "billing") => (
    <form onSubmit={handleAddAddress} className="space-y-6 p-6 bg-white rounded-xl shadow-md border border-gray-200">
      <div className="space-y-2">
        ...
        <Input
          ...
          className={`rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${addressFormErrors.street ? "border-red-500 focus:ring-red-500" : ""}`}
          required
        />
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className={`rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${addressFormErrors.city ? "border-red-500 focus:ring-red-500" : ""}`}
          required
        />
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className={`rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${addressFormErrors.state ? "border-red-500 focus:ring-red-500" : ""}`}
          required
        />
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className={`rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${addressFormErrors.pincode ? "border-red-500 focus:ring-red-500" : ""}`}
          maxLength={6}
          required
        />
      ...
      <div className="flex space-x-4 pt-4">
        ...
        <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">
          Save Address
        </Button>
      </div>
    </form>
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="text-center bg-white p-8 rounded-xl shadow-lg max-w-md w-full border border-gray-200">
          ...
          <Link href="/cart">
            <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Return to Cart</Button>
          </Link>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
                ...
                <div key={step} className="flex flex-col items-center">
                  <div
                    className={`flex items-center justify-center w-12 h-12 rounded-full ${
                      activeStep === step ? "bg-blue-600 text-white shadow-md" : "bg-white text-gray-500 border-2 border-gray-200"
                    } text-lg font-semibold`}
                  >
                    {index + 1}
                  </div>
                  <span
                    className={`mt-2 text-sm font-medium ${activeStep === step ? "text-blue-600" : "text-gray-600"}`}
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <Card className={`transition-all duration-300 ease-in-out ${activeStep !== "items" ? "hidden lg:block opacity-50" : "opacity-100"} bg-white rounded-xl shadow-lg border border-gray-200`}>
              ...
              <CardContent className="p-6">
                <div className="space-y-6">
                  {selectedItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="flex flex-col sm:flex-row gap-6 border-b border-gray-100 pb-6 last:border-b-0 last:pb-0">
                              ...
                              {isDesignable
                                ? viewMode === "apparel"
                                  ? designItem.designs?.map((design, index) => (
                                      <button
                                        key={index}
                                        className={`relative w-full aspect-square cursor-pointer rounded-md overflow-hidden shadow-sm border border-gray-200 hover:scale-105 ${index === mainDesignIndex ? "ring-2 ring-blue-500" : ""}`}
                                  ...
                                  : currentDesign?.uploadedImages?.map((image, index) => (
                                      <button
                                        key={index}
                                        className={`relative w-full aspect-square rounded-md overflow-hidden shadow-sm border border-gray-200 hover:scale-105 ${index === currentUploadedImageIndex ? "ring-2 ring-blue-500" : ""}`}
                                        onClick={() => setCurrentImageIndex((prev) => ({ ...prev, [item.id]: index }))}
                                        aria-label={`View uploaded image ${index + 1}`}
                                      >
                                        <Image src={image || "/placeholder.svg"} alt={`Upload ${index + 1}`} fill sizes="25vw" style={{ objectFit: "cover" }} />
                                      </button>
                                    ))
                                : standardSides.map((side, index) => (
                                    ...
                                    >
                                      <Image src={side.url || "/placeholder.svg"} alt={`Side: ${side.side}`} fill sizes="25vw" style={{ objectFit: "cover" }} />
                                    </button>
                                  ))}
                            </div>
                          )}
                          ...
                          <div className="mt-2 space-y-2 text-sm text-gray-600">
                            <p>
                              {isDesignable
                                ...
                                : `Size: ${standardItem.selected_size || "N/A"}, Color: ${standardItem.selected_color || "N/A"}`}
                            </p>
                            <p>Quantity: {item.quantity}</p>
                            <p>HSN Code: {item.hsnCode || FIXED_HSN_CODE}</p>
                            <p>GST Rate: {item.gstPercentage || FIXED_GST_PERCENTAGE}%</p>
                          </div>
                ...
                <div className="mt-8 flex justify-end">
                  <Button onClick={() => setActiveStep("shipping")} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg shadow-md">
            ...
            <Card className={`transition-all duration-300 ease-in-out ${activeStep !== "shipping" ? "hidden lg:block opacity-50" : "opacity-100"} bg-white rounded-xl shadow-lg border border-gray-200`}>
              <CardHeader className="p-6 border-b border-gray-100">
                <CardTitle className="text-2xl font-semibold text-gray-900 flex items-center">
                ...
                {isAddingShipping ? (
                  ...
                  <div className="space-y-6">
                    <div className="space-y-4">
                      ...
                      {addresses.filter((addr) => addr.address_type === "shipping").length === 0 ? (
                        <div className="bg-gray-50 p-6 rounded-lg text-center border border-gray-200">
                          ...
                          <Button variant="outline" onClick={() => setIsAddingShipping(true)} className="border-blue-500 text-blue-600 hover:bg-blue-50 rounded-lg">
                            <Plus className="h-4 w-4 mr-2" /> Add Shipping Address
                          </Button>
                        </div>
                      ) : (
                        <>
                          <RadioGroup value={selectedShippingAddressId} onValueChange={setSelectedShippingAddressId} className="space-y-4">
                            {addresses.filter((addr) => addr.address_type === "shipping").map((addr) => (
                              <div
                                key={addr.id}
                                className={`flex items-start space-x-4 p-4 rounded-lg border-2 ${
                                  selectedShippingAddressId === addr.id ? "border-blue-500 bg-blue-50 shadow-md" : "border-gray-200 hover:border-gray-300 bg-white"
                                }`}
                              >
                                <RadioGroupItem value={addr.id} id={`shipping-${addr.id}`} className="mt-1 text-blue-600" />
                                <div className="flex-1">
                                  <Label
                                    htmlFor={`shipping-${addr.id}`}
                                    className={`flex items-center text-base font-semibold ${selectedShippingAddressId === addr.id ? "text-blue-600" : "text-gray-900"}`}
                                  >
                                    <Home className="h-5 w-5 mr-2" />
                                    Shipping Address
                                    {selectedShippingAddressId === addr.id && <Badge className="ml-2 bg-blue-100 text-blue-600 border-blue-200">Selected</Badge>}
                                  </Label>
                          ...
                          <Button variant="outline" size="sm" onClick={() => setIsAddingShipping(true)} className="mt-2 border-blue-500 text-blue-600 hover:bg-blue-50 rounded-lg">
                      ...
                      {isAddingBilling ? (
                        ...
                        <>
                          ...
                          {selectedBillingAddressId !== selectedShippingAddressId && (
                            <RadioGroup value={selectedBillingAddressId} onValueChange={setSelectedBillingAddressId} className="space-y-4">
                              {addresses.filter((addr) => addr.address_type === "billing").map((addr) => (
                                <div
                                  key={addr.id}
                                  className={`flex items-start space-x-4 p-4 rounded-lg border-2 ${
                                    selectedBillingAddressId === addr.id ? "border-blue-500 bg-blue-50 shadow-md" : "border-gray-200 hover:border-gray-300 bg-white"
                                  }`}
                                >
                                  <RadioGroupItem value={addr.id} id={`billing-${addr.id}`} className="mt-1 text-blue-600" />
                                  <div className="flex-1">
                                    <Label
                                      htmlFor={`billing-${addr.id}`}
                                      className={`flex items-center text-base font-semibold ${selectedBillingAddressId === addr.id ? "text-blue-600" : "text-gray-900"}`}
                                    >
                                      <Building className="h-5 w-5 mr-2" />
                                      Billing Address
                                      {selectedBillingAddressId === addr.id && <Badge className="ml-2 bg-blue-100 text-blue-600 border-blue-200">Selected</Badge>}
                                    </Label>
                                    <p className="mt-1 text-gray-700 text-sm">
                                      {addr.street}, {addr.city}, {addr.state} - {addr.pincode}
                                    </p>
                          ...
                          <Button variant="outline" size="sm" onClick={() => setIsAddingBilling(true)} className="mt-2 border-blue-500 text-blue-600 hover:bg-blue-50 rounded-lg">
                    ...
                    <div className="mt-8 flex justify-between">
                      <Button variant="outline" onClick={() => setActiveStep("items")} className="border-gray-300 text-gray-700 hover:bg-gray-100 rounded-lg px-6 py-2">
            ...
            <Card className={`transition-all duration-300 ease-in-out ${activeStep !== "payment" ? "hidden lg:block opacity-50" : "opacity-100"} bg-white rounded-xl shadow-lg border border-gray-200`}>
              <CardHeader className="p-6 border-b border-gray-100">
                <CardTitle className="text-2xl font-semibold text-gray-900 flex items-center">
              ...
              <CardContent className="p-6">
                <div className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <RadioGroup value={paymentMethod} onValueChange={(value) => setPaymentMethod(value as "phonepe")} className="space-y-4">
                      <div
                      ...
                      >
                        ...
                        <Label htmlFor="phonepe" className={`flex items-center text-base font-semibold ${paymentMethod === "phonepe" ? "text-blue-600" : "text-gray-900"}`}>
                  ...
                  <div className="mt-8 flex justify-between">
                    <Button variant="outline" onClick={() => setActiveStep("shipping")} className="border-gray-300 text-gray-700 hover:bg-gray-100 rounded-lg px-6 py-2">
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  <div className="flex justify-between text-sm text-gray-600">
                    ...
                    <span className="font-medium text-gray-900">₹{orderTotals.subtotal}</span>
                  </div>
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>Shipping</span>
                    <span className="font-medium text-gray-900">₹{orderTotals.shippingCost}</span>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm font-semibold text-gray-700">
                      <span>Tax Total</span>
                      <span className="text-gray-900">₹{orderTotals.taxAmount}</span>
                    </div>
                        ...
                        <div key={index} className="space-y-1">
                          ...
                          {tax.cgst > 0 && (
                            <div className="flex justify-between text-gray-600">
                              <span>CGST ({tax.cgstPercentage?.toFixed(1)}%)</span>
                              <span>₹{tax.cgst}</span>
                            </div>
                          )}
                          {tax.sgst > 0 && (
                            <div className="flex justify-between text-gray-600">
                              <span>SGST ({tax.sgstPercentage?.toFixed(1)}%)</span>
                              <span>₹{tax.sgst}</span>
                            </div>
                          )}
                          {tax.igst > 0 && (
                            <div className="flex justify-between text-gray-600">
                              <span>IGST ({tax.tax_percentage.toFixed(1)}%)</span>
                              <span>₹{tax.igst}</span>
                            </div>
                          )}
                        </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-xl font-bold text-blue-600">₹{orderTotals.total}</span>
                  </div>
                  <div className="pt-4">
                    {activeStep === "items" && (
                      <Button onClick={() => setActiveStep("shipping")} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg shadow-md">
                ...
                <div className="mt-6 text-xs text-gray-500 space-y-2">
                  ...
                  <p>All prices are inclusive of applicable taxes unless otherwise stated.</p>
                </div>
              </CardContent>
            </Card>
          </div>
