...
const OrderPage = () => {
  ...
  >({});
  const [hsnTaxDetails, setHsnTaxDetails] = useState<any[]>([]);
  ...
  // Fetch product details for standardCartItems
  const fetchProductDetails = async (productId: string) => {
    try {
      ...
      return { hsnCode: "0000", gstPercentage: 18 }; // Fallback values
    }
  };

  const standardCartItems = useMemo(() => getStandardCartItems(), []);
  ...
  }, [standardCartItems, authToken]);

  const enrichedDesignableCartItems = useMemo(() => {
    return designableCartItems.map((item) => ({
      ...item,
      hsnCode: "1234", // Fixed HSN for designable items (customize as needed)
      gstPercentage: 12, // Fixed GST for designable items (customize as needed)
    }));
  }, [designableCartItems]);

  const allCartItems = useMemo(
    () => [...enrichedDesignableCartItems, ...enrichedStandardCartItems],
    [enrichedDesignableCartItems, enrichedStandardCartItems]
  );
  ...
  useEffect(() => {
    const fetchAuthToken = async () => {
      try {
        const response = await fetch("/api/auth-token"); // Calls our API
  ...
  useEffect(() => {
    ...
    if (addresses.length > 0 && !selectedBillingAddressId) {
      ...
      if (
        ...
        selectedBillingAddressId !== billingAddresses[0].id
      ) {
  ...
  const calculateTotals = () => {
    const hsnMap = new Map<
      string,
      { totalPrice: number; gstPercentage: number }
    >();
    selectedItems.forEach((item) => {
      ...
      const hsnCode = item.hsnCode || "0000";
      const gstPercentage = item.gstPercentage || 18;

      if (!hsnMap.has(hsnCode)) {
        hsnMap.set(hsnCode, { totalPrice: 0, gstPercentage });
      }
      const hsnData = hsnMap.get(hsnCode)!;
      hsnData.totalPrice += itemTotal;
    });

    const shippingFixedPrice = 50;
    const shippingIncrementalPrice = 20;
    const totalWeightInGrams = selectedItems.reduce((sum, item) => {
      const weight = item.weightInGrams || 100;
      return sum + weight * item.quantity;
    }, 0);
    let shippingCost = 0;
    if (totalWeightInGrams > 0) {
      const fullSlabs = Math.floor(totalWeightInGrams / 500);
      const remainingWeight = totalWeightInGrams % 500;
      shippingCost =
        fullSlabs >= 1
          ? shippingFixedPrice +
            (fullSlabs - 1) * shippingIncrementalPrice +
            (remainingWeight > 50 ? shippingIncrementalPrice : 0)
          : shippingFixedPrice;
    }
    ...
    hsnMap.forEach((hsnData, hsnCode) => {
      const { totalPrice, gstPercentage } = hsnData;
      ...
      taxDetails.push({
        hsnCode,
        taxPayableAmount: Math.round(gstAmount),
        cgstvalue: cgst,
        sgstvalue: sgst,
        igstvalue: igst,
      });
    ...
    const shippingGstPercentage = 18;
    const shippingBaseAmount = shippingCost / (1 + shippingGstPercentage / 100);
    const shippingGstAmount = shippingCost - shippingBaseAmount;
    let shippingCgst = 0,
      shippingSgst = 0,
      shippingIgst = 0;
    if (isSelfState) {
      shippingCgst = Math.round(shippingGstAmount / 2);
      shippingSgst = Math.round(shippingGstAmount / 2);
    } else {
      shippingIgst = Math.round(shippingGstAmount);
    }
    totalTaxAmount += shippingGstAmount;
    taxDetails.push({
      hsnCode: "9965",
      taxPayableAmount: Math.round(shippingGstAmount),
      cgstvalue: shippingCgst,
      sgstvalue: shippingSgst,
      igstvalue: shippingIgst,
    });

    setHsnTaxDetails(taxDetails);

    const subtotal = Array.from(hsnMap.values()).reduce(
      (sum, hsn) => sum + hsn.totalPrice,
      0
    );
                      ...
                      >
                        ...
                        <div className="sm:w-2/3 flex flex-col">
                          ...
                          <div className="mt-2 space-y-2 text-sm text-gray-600">
                            ...
                            <p>GST Rate: {item.gstPercentage}%</p>
                          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-gray-700 bg-gray-50 p-3 rounded-lg">
                      {hsnTaxDetails.map((tax, index) => (
                        <div key={index} className="mb-2 last:mb-0">
                          <div className="flex justify-between font-medium text-gray-800">
                            <span>HSN {tax.hsnCode}</span>
                            <span>₹{tax.taxPayableAmount}</span>
                          </div>
                          {tax.cgstvalue > 0 && (
                            <div className="flex justify-between text-gray-600">
                              <span>
                                ...
                                {(
                                  (tax.cgstvalue /
                                    (tax.taxPayableAmount || 1)) *
                                  100
                                ).toFixed(1)}
                                %)
                              </span>
                              <span>₹{tax.cgstvalue}</span>
                            </div>
                          )}
                          {tax.sgstvalue > 0 && (
                            <div className="flex justify-between text-gray-600">
