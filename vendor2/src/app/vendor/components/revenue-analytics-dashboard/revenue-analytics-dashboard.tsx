...
export default function RevenueAnalytics() {
  ...
  const { data, isLoading, isError } = useVendorAnalytics(vendorId, selectedMonth);
  const months = ["All", "Dec", "Jan", "Feb", "Mar"];

  // Guard clause for missing vendorId
  if (!vendorId) {
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: { position: "top" as const, align: "end" as const, labels: { boxWidth: 12, usePointStyle: true, pointStyle: "circle", color: chartColors.text, font: { family: "'Inter', sans-serif", size: 12 }, padding: 16 } },
      tooltip: { backgroundColor: chartColors.neutral, titleColor: chartColors.background, bodyColor: chartColors.background, titleFont: { family: "'Inter', sans-serif", size: 14, weight: "bold" as const }, bodyFont: { family: "'Inter', sans-serif", size: 12 }, padding: 12, cornerRadius: 8, displayColors: true, boxWidth: 8, boxHeight: 8, usePointStyle: true, borderColor: "rgba(255, 255, 255, 0.1)" },
    },
    scales: {
      x: { grid: { display: false, drawBorder: false }, ticks: { color: chartColors.textLight, font: { family: "'Inter', sans-serif", size: 11 } } },
      y: { grid: { color: chartColors.border, drawBorder: false }, ticks: { color: chartColors.textLight, font: { family: "'Inter', sans-serif", size: 11 }, callback: (value) => (value >= 1000 ? `$${value / 1000}k` : `$${value}`) } },
    },
  };
  ...
  const horizontalBarOptions = {
    ...
    scales: { ...chartOptions.scales, x: { ...chartOptions.scales.x, ticks: { ...chartOptions.scales.x.ticks, callback: (value) => (value >= 1000 ? `$${value / 1000}k` : `$${value}`) } } },
  ...
  const pieOptions = {
    ...
    maintainAspectRatio: false,
    plugins: {
      legend: { position: "right" as const, labels: { boxWidth: 12, usePointStyle: true, pointStyle: "circle", color: chartColors.text, font: { family: "'Inter', sans-serif", size: 12 }, padding: 16 } },
    ...
    <div className="container flex-1 space-y-6 p-4 md:p-8 bg-base-200">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="join">
            {months.map((month) => (
              <button key={month} className={`btn btn-sm join-item ${selectedMonth === month ? "btn-primary" : "btn-ghost"}`} onClick={() => setSelectedMonth(month)}>
                {month}
              </button>
            ))}
          </div>
      ...
      <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" variants={container} initial="hidden" animate="show">
          ...
          <div className="card card-border bg-base-100">
            <div className="card-body p-4">
              ...
              <div className="text-3xl font-bold text-primary mt-2">
                $<AnimatedCounter value={Number(data.commission.total_vendor_revenue)} formatValue={(value) => value.toLocaleString(undefined, { minimumFractionDigits: 2 })} />
              </div>
              <div className="text-xs text-base-content/60 mt-2 flex items-center">
                <span className="badge badge-primary badge-xs mr-1"></span>
                Final: $<AnimatedCounter value={Number(data.commission.final_vendor_revenue)} formatValue={(value) => value.toLocaleString(undefined, { minimumFractionDigits: 2 })} />
              </div>
            </div>
          </div>
        </motion.div>
          ...
          <div className="card card-border bg-base-100">
            <div className="card-body p-4">
              ...
              <div className="text-xs text-base-content/60 mt-2 flex items-center">
                ...
                Total: $<AnimatedCounter value={Number(data.commission.total_admin_commission)} formatValue={(value) => value.toLocaleString(undefined, { minimumFractionDigits: 2 })} />
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div variants={item}>
          <div className="card card-border bg-base-100">
            <div className="card-body p-4">
              ...
              <div className="text-3xl font-bold text-error mt-2">{data.engagement.cart_abandonment.abandonment_rate}%</div>
              <div className="text-xs text-base-content/60 mt-2 flex items-center">
